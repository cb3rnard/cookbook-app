import { ConnectionState, SessionState } from '@src/types/states.js';
import {
  createContext,
  useCallback,
  useContext,
...
export function ApiProvider({ children }: { children: React.ReactNode }) {
  ...
  const session = useObservableState<SessionState>(apiService.sessionStore);
  const connection = useObservableState<ConnectionState>(
    apiService.connectionStore,
  );
  ...
  }, [status, currentStatus, connecting, addNotice]);
  ...
  const login = useCallback(
    async (
      ...
      return apiService.operations.auth.login(apiUrl, email, password);
    },
    [apiService],
  );
