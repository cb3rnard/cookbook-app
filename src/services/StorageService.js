...
class StorageDatabase extends Dexie {
  constructor() {
    ...
    this.version(1).stores({
      recipes: '$$uuid, name, tested, favorite, *tags, timePreparation, timeCook, imageUuid, portions, difficulty, dateAdd, dateModify, version, lastSyncDate, isDirty, dateDeleted',
      ingredients: '$$uuid, &name, type, dateAdd, dateModify, version, lastSyncDate, isDirty, dateDeleted',
      recipeIngredients: '$$uuid, ingredientUuid, recipeUuid, quantity, unit',
      types: '$$uuid, &name, parentUuid, dateAdd, dateModify, version, lastSyncDate, isDirty, dateDeleted',
...
export const database = new StorageDatabase("cookbookDB", { addons: [dexieObservable, dexieSyncable] });
  ...
  static async requestPersistentStorage() {
    if ('storage' in navigator && 'persist' in navigator.storage) {
      try {
        ...
        console.warn('Erreur lors de la demande de stockage persistent:', error);
        return false;
      }
    }
    return false;
