import { defaultSessionState, SessionState } from '@src/types/states';
...
export class SessionStore extends BaseStore<SessionState> {
  ...
  saveSession(user: { id: string; email: string }, apiUrl = '') {
    if (!user.id || !apiUrl) {
    ...
    this._persistSession({
      ...
      lastSyncDate: this._state.lastSyncDate,
      active: true,
    });
  }
   ...
   * @private
   */
  _persistSession(newState: Partial<SessionState>) {
