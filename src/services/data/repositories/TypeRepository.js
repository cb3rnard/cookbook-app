...
export class TypeRepository extends BaseSyncRepository {
  ...
  async getUsed() {
    try {
      ...
      console.error(
        "Erreur lors de la récupération des types utilisés:",
        error,
      );
      throw error;
    }
  ...
  async delete(typeUuid, synced = false) {
    try {
      const relatedRecipes = await this.table.getRelatedRecipesUuids(typeUuid);
      ...
      this.eventBus.emit("type:deleted", typeUuid);
