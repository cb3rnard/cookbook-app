import { ApiErrorSummary, JSONResponse } from '@src/types/api';
...
export class ApiPlatformError extends Error {
  ...
  serverTrace: any | null;

  violations: any[] | null; // Pour les erreurs de validation
  hydraContext: string;

  rawResponse: Partial<JSONResponse>;
  category:
    | 'authentication'
    | 'authorization'
    | 'not_found'
  ...
  static async fromResponse(
    response: Response,
    context = '',
  ): Promise<ApiPlatformError> {
    let responseData: Partial<JSONResponse> = {};

    try {
      // Tenter de parser le JSON même en cas d'erreur HTTP
      responseData = (await response.json()) as JSONResponse;
   ...
   * @static
   */
  static isApiPlatformError(responseData: Partial<JSONResponse>): boolean {
    return !!(
      responseData &&
