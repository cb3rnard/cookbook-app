 ...
 */
import { ApiPlatformError } from '../utils/ApiPlatformError.js';
...
export class ApiErrorHandler {
    ...
    static handleError(error, context = {}) {
        const result = {
            ...
            category: 'unknown',
            strategy: this.STRATEGIES.unknown,
            userMessage: null,
        ...
        if (error instanceof ApiPlatformError) {
            ...
            result.strategy = this.STRATEGIES[error.category] || this.STRATEGIES.unknown;
            ...
            result.actions.shouldRetry = result.strategy.shouldRetry && error.isRetryable;
     ...
     * @private
     */
    static _buildUserMessage(apiError) {
        ...
        if (apiError.category === 'validation' && apiError.violations) {
            ...
            if (validationErrors.length > 0) {
                const fields = validationErrors.map(v => v.property).filter(Boolean).join(', ');
     ...
     * @private
     */
    static _handleStandardError(error) {
    ...
    static toUiError(error, context = {}) {
        ...
        return {
            ...
            details: process.env.NODE_ENV === 'development' ? handled.technicalDetails : null
        };
    }
}