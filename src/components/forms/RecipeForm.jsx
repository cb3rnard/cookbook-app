...
export const RecipeForm = ({
  ...
  useEffect(() => {
    const fetchRecipe = async () => {
      ...
      try {
        setIsLoading(true);
        const fetchedRecipe = await repository.get(
          recipeUuid,
          {
            construct: true,
            withImage: true,
  ...
  const addIngredient = (e) => {
    ...
    setFormData((prev) => {
      ...
      updated.ingredients = [
        ...updated.ingredients,
        { ingredientUuid: null, name: "", quantity: 0, unit: "", note: "" },
      ];
      return updated;
    });
  };
  ...
  const handlePreSave = async (formData) => {
    setFormData((prev) => {
      const updated = prev.clone();
      updated.ingredients = formData.ingredients.filter(
        (ing) => ing.ingredientUuid || ing.note,
      ); // Filtrer les ingrédients sans UUID
      return updated;
    });
    return formData;
  };
          ...
          <>
            <Flex justify="between" align="center" className={styles.header}>
              <Flex direction="column" gap="2">
                <Heading as="h1">
                  {recipeUuid
                    ? `Modifier la recette "${formData.name}"`
                    : recipeData
                      ? `Éditer "${formData.name || "Recette importée"}"`
                ...
                {formData.types && (
                  <Box>
                    ...
                    <EntitySelect
                      ...
                      selectedUuids={formData.types.map(
                        (type) => type.typeUuid,
                      )}
                      onChange={(uuids) => {
                        setFormData((prev) => {
